...
static void UtInsertData(sqlite3 *db, int insertNum)
{
...
/**
 * @tc.name: BinlogReplayTest004
 * @tc.desc: Test batch replay binlog
 * @tc.type: FUNC
 */
HWTEST_F(SqliteBinlogTest, BinlogReplayTest004, TestSize.Level0)
{
    /**
     * @tc.steps: step1. open db and set binlog
     * @tc.expected: step1. ok
    ...
    ASSERT_NE(backupDb, nullptr);
    /**
     * @tc.steps: step4. binlog replay to write into backup db, wait 10ms per transaction
     * @tc.expected: step4. replay time is greater than 100ms
     */
    BinlogReplayConfig replayConfig = {.batchNum = 1, .batchWaitTime = 10};
    struct timespec sTime, eTime;
    (void)clock_gettime(CLOCK_MONOTONIC, &sTime);
    EXPECT_EQ(sqlite3_replay_binlog_v2(db, backupDb, &replayConfig), SQLITE_OK);
    (void)clock_gettime(CLOCK_MONOTONIC, &eTime);
    double elapsedMs = (eTime.tv_sec - sTime.tv_sec) * 1000.0 + (eTime.tv_nsec - sTime.tv_nsec) / 1000000;
    double replayWaitTime = ((double)dataNum) / replayConfig.batchNum * replayConfig.batchWaitTime;
    EXPECT_TRUE(elapsedMs > replayWaitTime);
    /**
     * @tc.steps: step5. check db count
     * @tc.expected: step5. both db have TEST_DATA_COUNT
     */
    EXPECT_EQ(UtGetRecordCount(db), dataNum);
    EXPECT_EQ(UtGetRecordCount(backupDb), dataNum);
    sqlite3_close_v2(db);
    sqlite3_close_v2(backupDb);
}

/**
 * @tc.name: BinlogReplayTest005
 * @tc.desc: Test batch replay binlog
 * @tc.type: FUNC
 */
HWTEST_F(SqliteBinlogTest, BinlogReplayTest005, TestSize.Level0)
{
    /**
     * @tc.steps: step1. open db and set binlog
     * @tc.expected: step1. ok
    ...
    UtEnableBinlog(db);
    /**
     * @tc.steps: step2. binlog replay to write into backup db
    ...
    ASSERT_NE(backupDb, nullptr);
    /**
     * @tc.steps: step3. Insert records with single quotes
     * @tc.expected: step3. Execute successfully
     */
    int dataNum = 10;
    UtInsertData(db, dataNum);
    /**
     * @tc.steps: step4. binlog replay to write into backup db, wait 50ms each 5 transactions
     * @tc.expected: step4. replay time is greater than 100ms
     */
    BinlogReplayConfig replayConfig = {.batchNum = 5, .batchWaitTime = 50};
    struct timespec sTime, eTime;
    (void)clock_gettime(CLOCK_MONOTONIC, &sTime);
    EXPECT_EQ(sqlite3_replay_binlog_v2(db, backupDb, &replayConfig), SQLITE_OK);
    (void)clock_gettime(CLOCK_MONOTONIC, &eTime);
...
/**
 * @tc.name: BinlogReplayTest006
 * @tc.desc: Test batch replay binlog
 * @tc.type: FUNC
 */
HWTEST_F(SqliteBinlogTest, BinlogReplayTest006, TestSize.Level0)
{
    /**
     * @tc.steps: step1. open db and set binlog
     * @tc.expected: step1. ok
    ...
    ASSERT_NE(backupDb, nullptr);
    /**
     * @tc.steps: step3. Insert 10 records
     * @tc.expected: step3. Execute successfully
     */
    int dataNum = 10;
    UtInsertData(db, dataNum);
    /**
     * @tc.steps: step4. binlog replay to write into backup db, wait 200ms per transaction
     * @tc.expected: step4. replay time is less than 200ms
     */
    BinlogReplayConfig replayConfig = {.batchNum = 1, .batchWaitTime = 200};
    struct timespec sTime, eTime;
    (void)clock_gettime(CLOCK_MONOTONIC, &sTime);
    EXPECT_EQ(sqlite3_replay_binlog_v2(db, backupDb, &replayConfig), SQLITE_OK);
    (void)clock_gettime(CLOCK_MONOTONIC, &eTime);
    double elapsedMs = (eTime.tv_sec - sTime.tv_sec) * 1000.0 + (eTime.tv_nsec - sTime.tv_nsec) / 1000000;
    double replayWaitTime = ((double)dataNum) * replayConfig.batchWaitTime;
    EXPECT_TRUE(elapsedMs < replayWaitTime);
    /**
     * @tc.steps: step5. check db count
     * @tc.expected: step5. both db have TEST_DATA_COUNT
...
/**
 * @tc.name: BinlogReplayTest007
 * @tc.desc: Test replay binlog with transaction
 * @tc.type: FUNC
 */
HWTEST_F(SqliteBinlogTest, BinlogReplayTest007, TestSize.Level0)
{
    /**
     * @tc.steps: step1. open db and set binlog
     * @tc.expected: step1. ok
    ...
    ASSERT_NE(backupDb, nullptr);
    /**
     * @tc.steps: step3. Insert 10 records in transaction
     * @tc.expected: step3. Execute successfully
     */
    EXPECT_EQ(sqlite3_exec(db, "begin;", nullptr, nullptr, nullptr), SQLITE_OK);
    int dataNum = 10;
    ...
    EXPECT_EQ(sqlite3_exec(db, "commit;", nullptr, nullptr, nullptr), SQLITE_OK);
    /**
     * @tc.steps: step4. binlog replay to write one transaction into backup db, wait 10ms per transaction
     * @tc.expected: step4. replay time is greater than 10ms, but less than 100ms
     */
    BinlogReplayConfig replayConfig = {.batchNum = 1, .batchWaitTime = 10};
    struct timespec sTime, eTime;
    (void)clock_gettime(CLOCK_MONOTONIC, &sTime);
    ...
    (void)clock_gettime(CLOCK_MONOTONIC, &eTime);
    double elapsedMs = (eTime.tv_sec - sTime.tv_sec) * 1000.0 + (eTime.tv_nsec - sTime.tv_nsec) / 1000000;
    EXPECT_TRUE(elapsedMs > replayConfig.batchWaitTime && elapsedMs < dataNum * replayConfig.batchWaitTime);
    /**
     * @tc.steps: step5. check db count
     * @tc.expected: step5. both db have TEST_DATA_COUNT
     */
